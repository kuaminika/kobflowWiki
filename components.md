...
- **phpGateway** — currently handles authentication and grants access 
  to the .NET services.
- **react website** — the frontend client.
...
the react website no longer calls `.NET Services` or `Node.js Services` 
directly. `phpGateway` also delegates authentication to `userModule` 
